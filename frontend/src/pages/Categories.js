...
import { categoryApi, getLocalCategories, saveLocalCategory, renameLocalCategory, removeLocalCategory } from "../services/api";

const defaults = ["Testing", "Cloud", "Development", "Oracle SQL", "UI/UX Design", "Data Analytics / Data Science", "Fresher Placement", "Experienced Placement", "Documents / Test Only"];
const toRecord = item => typeof item === "string" ? {id: item, name: item} : item;

export default function Categories() {
  const [rows, setRows] = useState([...defaults, ...getLocalCategories().filter(item => !defaults.includes(item))].map(name => ({id: name, name})));
  const [query, setQuery] = useState("");
  const [name, setName] = useState("");
  const [editing, setEditing] = useState(null);
  const [formOpen, setFormOpen] = useState(false);
  useEffect(() => { categoryApi.list().then(response => { const values = (response.data.results || response.data || []).map(toRecord); setRows([...new Map([...defaults.map(item => ({id: item, name: item})), ...values, ...getLocalCategories().map(item => ({id: item, name: item}))].map(item => [item.name, item])).values()]); }).catch(() => {}); }, []);
  const visible = rows.filter(category => category.name.toLowerCase().includes(query.toLowerCase()));
  async function add(event) { event.preventDefault(); const value = name.trim(); if (!value || rows.some(item => item.name.toLowerCase() === value.toLowerCase())) return; await categoryApi.create({name: value}); saveLocalCategory(value); setName(""); setFormOpen(false); await reload(); }
  async function update(event) { event.preventDefault(); const value = name.trim(); if (!value || !editing) return; await categoryApi.update(editing.id, {name: value}); renameLocalCategory(editing.name, value); setEditing(null); setName(""); await reload(); }
  async function reload() { const response = await categoryApi.list(); setRows((response.data.results || response.data || []).map(toRecord)); }
  async function remove(category) { if (!window.confirm(`Delete category ${category.name}?`)) return; await categoryApi.remove(category.id); removeLocalCategory(category.name); setRows(rows.filter(item => item.id !== category.id)); }
